...
function ListagemProcuras() {
  ...
  const cadastrar = () => {
    navigate(`/cadastro-procura`);
  };

  const editar = (id) => {
    navigate(`/cadastro-procura/${id}`);
  };
  ...
  async function excluir(id) {
    ...
    await axios
      ...
      .then(function (response) {
        mensagemSucesso(`Procura excluída com sucesso!`);
        setDados(
          dados.filter((dado) => {
            return dado.id !== id;
          })
      ...
      .catch(function (error) {
        mensagemErro(`Erro ao excluir a Procura`);
      });
  }
  ...
  return (
    <div className='container'>
      <Card title='Listagem de Procuras'>
        <div className='row'>
...
export default ListagemProcuras;
